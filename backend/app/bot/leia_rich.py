...
def format_packages_menu_rich() -> str:
    ...
    for pkg in PACKAGES.values():
        tag = {
            "test_10": "проверка кассы, без доступа",
...
def format_package_pitch_rich(package_id: str, *, active: bool) -> str:
    ...
    body = {
        "test_10": (
            "Проверка Робокассы на 10 ₽. Доступа не даёт, пакет не активирует."
        ),
        "happy_woman": (
        ...
        "love_plus": (
            "Месяц безлимитных полных разборов по «Любви» и «Переписке». "
            "Для вопросов «что он думает», «вернётся ли», «что между вами в чате»."
        ),
        "vip": (
            "Месяц полного доступа ко всем темам — любовь, переписка, деньги, прогноз, "
