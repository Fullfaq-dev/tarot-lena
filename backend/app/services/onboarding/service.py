    ...
    async def start_or_resume(
        ...
        async with AsyncSessionLocal() as session:
            ...
            if user.is_onboarded:
                return (
                    "С возвращением! Можешь задать вопрос, выбрать расклад или посмотреть карту дня.",
                    user.id,
                    False,
                )
    ...
    async def handle_answer(
        ...
        async with AsyncSessionLocal() as session:
            ...
            return (
                ...
                "Со временем буду запоминать важные события, цели и людей, о которых ты рассказываешь.",
                user.id,
                True,
            )
