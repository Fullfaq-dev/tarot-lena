...
export function App() {
  ...
  async function calculate() {
    ...
    try {
      ...
      setScreen(6);
      if (r.mini?.free && sel.id === "daily") {
        /* карта дня без пейвола */
      }
    } catch (e) {
  ...
  return (
    <div className="app">
      {screen === 1 && (
        <>
          <div className="logo"><i>✦</i> Лея</div>
          <div className="h">Что тебя сейчас волнует?</div>
          <p className="sub">Выбери ситуацию — отвечу по картам или по дате рождения</p>
          <div className="tabs">
            <button className={`tab ${tab === "rel" ? "on" : ""}`} onClick={() => setTab("rel")}>Про отношения</button>
            <button className={`tab ${tab === "me" ? "on" : ""}`} onClick={() => setTab("me")}>Про меня</button>
          </div>
          <div className="cards">
            {visible.map((c) => (
              <button key={c.id} className="c" onClick={() => pickCard(c)}>
      ...
      {screen === 3 && sel && (
        <div className="calm">
          ...
          <p>Не торопись. Держи это в голове — и нажми, когда будешь готова.</p>
